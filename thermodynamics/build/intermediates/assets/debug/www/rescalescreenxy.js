// JavaScript Document
// グラフを描くキャンバスです。
var RescaleScreenXY=function(canvas_string,ww,hh,adjustW,ratio) {
	...
	if( adjustW > 0 ) {
		...
		this.canvas.height=wx*this.h*this.ratio/this.w;
	}
}

RescaleScreenXY.prototype={
    ...
    setScale:function() {
		...
		this.ctx.setTransform(sc,0,0,-sc*this.ratio,0,0);
...
// (x1,y1)から(x2,y2)へ線分を引く。
	...
	setXLabel:function(img) {
		this.xlabel=img;
	},
	setYLabel:function(img) {
		this.ylabel=img;
	},
	writeCoordinate:function() {
       ...
       ct.fill();
       if( this.ylabel != null ) {
		 ct.drawImage(this.ylabel,this.w*0.03,this.topy-this.w*0.1,this.w*0.05*this.ylabel.width/this.ylabel.height,this.w*0.05/this.ratio);	
       }
       ct.beginPath();
       ...
       ct.fill();
       if( this.xlabel != null ) {
		 ct.drawImage(this.xlabel,this.rightwx-this.w*0.15,this.w*0.03,this.w*0.05*this.xlabel.width/this.xlabel.height,this.w*0.05/this.ratio);
       }
