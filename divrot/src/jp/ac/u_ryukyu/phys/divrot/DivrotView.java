...
public class DivrotView extends DraggManageView {
	TextView l1,l2;
	DivrotGraphView graphview;
	
	public DivrotView(Context context,TextView ll1,TextView ll2,DivrotGraphView gv) {
		super(context);
		l1=ll1;
		l2=ll2;
		graphview=gv;
	...
	class RotSource extends Source {
		...
		public void writeP(Canvas cv) {
			...
			if( charge>0) {
				cv.drawLine(x.X()+0.3f*r, x.Y(), x.X()+0.7f*r, x.Y(), p);
				cv.drawLine(x.X()+0.3f*r, x.Y(), x.X()+0.5f*r, x.Y()-0.4f*r, p);
				cv.drawLine(x.X()+0.7f*r, x.Y(), x.X()+0.5f*r, x.Y()-0.4f*r, p);
			} else {
				cv.drawLine(x.X(), x.Y()-0.3f*r, x.X(), x.Y()-0.7f*r, p);
				cv.drawLine(x.X(), x.Y()-0.3f*r, x.X()+0.4f*r, x.Y()-0.5f*r, p);
				cv.drawLine(x.X(), x.Y()-0.7f*r, x.X()+0.4f*r, x.Y()-0.5f*r, p);
			}
		}
	}
