// JavaScript Document
// グラフを描くキャンバスです。
var GraphScreenXY=function(canvas_string,hh,ww,adjustW,syx) {
	this.funcList=new Array();
    RescaleScreenXY.call(this,canvas_string,hh,ww,adjustW,syx);
}


GraphScreenXY.prototype=Object.create(RescaleScreenXY.prototype);
GraphScreenXY.prototype.constructor=GraphScreenXY;

GraphScreenXY.prototype.clearFunction=function() {
	this.funcList=new Array();
}
GraphScreenXY.prototype.writeGraph=function() {
	this.clearALL();
	this.writeContents();
}
GraphScreenXY.prototype.writeContents=function() {
    this.writeCoordinate();
    this.writeFunction(this.samples);
}
GraphScreenXY.prototype.writeCoordinate=function() {
    var ct=this.ctx;
...
// 横軸、縦軸のラベル。
GraphScreenXY.prototype.xlabel=null;
GraphScreenXY.prototype.ylabel=null;
// ラベルになる画像をセットする。graphscreenは縦が反転しているので、画像も反転画像を用意する。
GraphScreenXY.prototype.setxlabel=function(f) { this.xlabel=new Image(); this.xlabel.src=f;}
GraphScreenXY.prototype.setylabel=function(f) { this.ylabel=new Image(); this.ylabel.src=f;}

// プロットする関数。
GraphScreenXY.prototype.func=function(x) { return x;}
GraphScreenXY.prototype.setFunction=function(f,col) {
	if( col == undefined ) {
...
// 設定しておいた関数のグラフを描く。
GraphScreenXY.prototype.writeFunction=function(num){
	if( num == undefined ) {
		num=100;
	}
    var ct=this.ctx;
...
GraphScreenXY.prototype.writeLinear=function(a,x0,b,c) {
	var ct=this.ctx;
	var x1=this.leftwx;
	var y1=a*(x1-x0)+b;
	var x2=this.rightwx;
