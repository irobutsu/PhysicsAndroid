...
abstract public class MovingObject {
	...
	public boolean tamatteru() {
		if( dragMan !=null) {
			return dragMan.tamatteru();
		} 
		return false;
	}
	public boolean startDragg(int i) {
		if( nowCanDrag ) {
			return startDragg(i, Vec2.zero);
		}
		return false;
	}
	public boolean startDragg(int i,Vec2 s) {
		if( dragMan != null ) {
			return dragMan.startDragg(i, s);
		}
		return false;
	}
	public boolean releaseDragg(){
		if( dragMan != null ) {
			return dragMan.releaseDragg();
		}
		return false;
	}
	...
	public boolean isDraggedBy(int ptID) {
		if( dragMan != null ) {
			return dragMan.isDraggedBy(ptID);
		}
		return false;
