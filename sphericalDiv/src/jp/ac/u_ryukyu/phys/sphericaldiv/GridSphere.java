...
import jp.ac.u_ryukyu.phys.tool3D.Hexahedron;
...
public class GridSphere extends MovingObject3D {
	...
	public GridSphere(float RR,int TTnum,int PPnum,int XXnum,float r,float g,float b,float a) {
		super(r,g,b,a);
		Xnum=XXnum;
		R=RR;
		deltaR=R*0.15f;
		...
		yt.setThetaPhiPts(FHPI, 0f);
		yt.translatePts(new Vec3(0,0,R));
		yp=new Yajirushi3D(0.4f, 0.03f, 0.2f, Xnum,0f,1f,1f,1f);
		yp.setThetaPhiPts(FHPI, FHPI);
		yp.translatePts(new Vec3(0,0,R));
	}
	public void setTPoint(int p) {
		TPoint=p;
	}
	public void upTPoint() {
		if( TPoint < Tnum-3) {
			TPoint++;
		}
	}
	public void downTPoint() {
		if( TPoint > 0) {
			TPoint --;
		}
	}
	...
	public void drawvecr(GL10 gl,float tt) {
		...
		yr.copyFromOrg();
		yr.translatePts(new Vec3(0,0,R+w*deltaR));
		
		int i,j;
		for(i=0; i<5 ;i++) {
			float theta=deltaTheta*(TPoint+1+0.2f*i+0.1f);
			for(j=0; j<5 ; j++ ) {
				float phi=(0.2f*j+0.1f)*deltaPhi;
	...
	public void drawvectheta(GL10 gl,float t) {
		...
		for(i=0; i<5 ;i++) {
			yt.copyFromOrg();
			yt.setThetaPhiPts(FHPI, 0f);
			yt.translatePts(new Vec3(0,0,R+(0.2f*i+0.1f)*deltaR));
			float theta=deltaTheta*(TPoint+1+w);
			for(j=0; j<5 ; j++ ) {
				float phi=(0.2f*j+0.1f)*deltaPhi;
...
public void drawvecphi(GL10 gl,float t) {
		...
		for(i=0; i<5 ;i++) {
			yp.copyFromOrg();
			yp.setThetaPhiPts(FHPI, FHPI);
			yp.translatePts(new Vec3(0,0,R+(0.2f*i+0.1f)*deltaR));
			float phi=deltaPhi*w;
			for(j=0; j<5 ; j++ ) {
				float theta=deltaTheta*(TPoint+1+0.2f*j+0.1f);
				yp.setThetaPhi(theta, phi);
				yp.draw(gl);
			}
		}
	}
	public void drawRoom(GL10 gl) {
		float R1=R+deltaR;
		float theta0=deltaTheta*(TPoint+1);
		float sintheta0=FloatMath.sin(theta0);
		float costheta0=FloatMath.cos(theta0);
		float theta1=deltaTheta*(TPoint+2);
		float sintheta1=FloatMath.sin(theta1);
		float costheta1=FloatMath.cos(theta1);
		float cosphi1=FloatMath.cos(deltaPhi);
		float sinphi1=FloatMath.sin(deltaPhi);
		Hexahedron room=new Hexahedron(
				new Vec3(R*sintheta0*cosphi1,R*sintheta0*sinphi1,R*costheta0), // new Vec3(0,1,0),
				new Vec3(R*sintheta1*cosphi1,R*sintheta1*sinphi1,R*costheta1), //new Vec3(1,1,0),
				new Vec3(R*sintheta1,0f,R*costheta1), //new Vec3(1,0,0),
				new Vec3(R*sintheta0,0f,R*costheta0), //new Vec3(0,0,0),
				new Vec3(R1*sintheta0,0f,R1*costheta0), //new Vec3(0,0,1),
				new Vec3(R1*sintheta1,0f,R1*costheta1), //new Vec3(1,0,1),
				new Vec3(R1*sintheta1*cosphi1,R1*sintheta1*sinphi1,R1*costheta1), //new Vec3(1,1,1),
				new Vec3(R1*sintheta0*cosphi1,R1*sintheta0*sinphi1,R1*costheta0), // new Vec3(0,1,1),
				1f,0f,0f,0.5f);
		room.draw(gl);
//		Hexahadron room=new Hexahadron(
//				new Vec3(R*sintheta0,0f,R*costheta0),
//				new Vec3(R*sintheta0*cosphi1,R*sintheta0*sinphi1,R*costheta0),
//				new Vec3(R*sintheta1,0f,R*costheta1),
//				new Vec3(R*sintheta1*cosphi1,R*sintheta1*sinphi1,R*costheta1),
//				
//		);
	}
}
