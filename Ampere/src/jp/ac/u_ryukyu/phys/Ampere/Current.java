...
public class Current extends MovingObject {
	private int q;
	...
	protected void write_sub(Canvas canvas,float xx,float yy) {
		...
		p.setStrokeWidth(2f);
		if( q>0) {
			canvas.drawCircle(xx,yy,0.3f*hankei,p);
		}
		if( q< 0 ) {
			canvas.drawLine(xx+hankei*0.8f,yy-hankei*0.8f,xx-hankei*0.8f,yy+hankei*0.8f,p);
			canvas.drawLine(xx+hankei*0.8f,yy+hankei*0.8f,xx-hankei*0.8f,yy-hankei*0.8f,p);
		}
