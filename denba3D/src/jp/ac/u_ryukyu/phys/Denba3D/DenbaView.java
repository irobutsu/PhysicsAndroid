...
public class DenbaView extends View {
	...
	protected void onDraw(Canvas canvas) {
       ...
       if( showVFlg && !Q1.isDragged() && !Q2.isDragged() ) {    
    	   ...
    	   float V2ofset=Q2.Q()*((float)Math.log(1000));
	...
	public boolean onTouchEvent(MotionEvent event) {
    	...
    	for(i=0; i<pointerCount ; i++ ) {
    		...
    		case MotionEvent.ACTION_MOVE:
    			if( Q1.isDraggedBy(i)) {
    				Q1.setPosition(mp[i]);
    				if(Q1.X()> w-Q1.R()) { Q1.setX(w-Q1.R());}
    				if(Q1.X()< Q1.R()) { Q1.setX(Q1.R());}
    				if(Q1.Y()> h-Q1.R()) { Q1.setY(h-Q1.R());}
    				if(Q1.Y()< Q1.R()) { Q1.setY(Q1.R());}
    				invalidate();
    			}
    			if( Q2.isDraggedBy(i)) {
    				Q2.setPosition(mp[i]);
    				if(Q2.X()> w-Q2.R()) { Q2.setX(w-Q2.R());}
    				if(Q2.X()< Q2.R()) { Q2.setX(Q2.R());}
    				if(Q2.Y()> h-Q2.R()) { Q2.setY(h-Q2.R());}
    				if(Q2.Y()< Q2.R()) { Q2.setY(Q2.R());}
    				invalidate();
    			}
    			break;
