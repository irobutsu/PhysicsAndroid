...
public class MonoPole {
	...
	public MonoPole(int Q,int R,int cc,Vec2 xx,float a) {
		q=Q;
		hankei=R;
		cl=cc;
		x=xx;
	...
	protected void write(Canvas canvas) {
		...
		p.setStrokeWidth(2f);
		if( q>0) {
			canvas.drawLine(x.X()-hankei*0.5f,x.Y()-hankei*0.5f,x.X()-hankei*0.5f,x.Y()+hankei*0.5f,p);
			canvas.drawLine(x.X()+hankei*0.5f,x.Y()-hankei*0.5f,x.X()+hankei*0.5f,x.Y()+hankei*0.5f,p);
			canvas.drawLine(x.X()-hankei*0.5f,x.Y()-hankei*0.5f,x.X()+hankei*0.5f,x.Y()+hankei*0.5f,p);
		}
		if( q< 0 ) {
			canvas.drawLine(x.X()-0.5f*hankei,x.Y()-0.5f*hankei,x.X()+0.5f*hankei,x.Y()-0.5f*hankei,p);
			canvas.drawLine(x.X()-0.5f*hankei,x.Y()-0.5f*hankei,x.X()-0.5f*hankei,x.Y(),p);
			canvas.drawLine(x.X()-0.5f*hankei,x.Y(),x.X()+0.5f*hankei,x.Y(),p);
			canvas.drawLine(x.X()+0.5f*hankei,x.Y()+0.5f*hankei,x.X()+0.5f*hankei,x.Y(),p);
			canvas.drawLine(x.X()-0.5f*hankei,x.Y()+0.5f*hankei,x.X()+0.5f*hankei,x.Y()+0.5f*hankei,p);
		}
	}
